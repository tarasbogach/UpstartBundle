...
INFO
        ...
        $nativeChildren = $defaultChildren
            ...
            ->validate()->always(function($v){
                foreach(['env','export','normalExit','cgroup','limit'] as $key){
                    if(!$v[$key]){unset($v[$key]);}
                }
                return $v;
            })->end()
...
INFO
        ...
        $jobNativeChildren = $jobPrototypeChildren
            ->arrayNode('native')
                ->validate()->always(function($v){
                    foreach(['env','export','normalExit','cgroup','limit','emits'] as $key){
                        if(!$v[$key]){unset($v[$key]);}
                    }
                    return $v;
                })->end()
...
INFO
                ...
                ->validate()->always($this->ifNotThenRemove())->end()
                ->prototype('scalar')->end()
            ->end()
            ->scalarNode('version')
                ->info(<<<INFO
